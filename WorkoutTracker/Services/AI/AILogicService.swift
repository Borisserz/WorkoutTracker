...
let langInstruction = isRussian ? "ОТВЕЧАЙ СТРОГО НА РУССКОМ ЯЗЫКЕ (но значения ключей JSON оставь английскими). Будь кратким и мотивирующим." : "REPLY STRICTLY IN ENGLISH. Be brief and motivational."
let systemPrompt = """
...
let requestBody = GeminiRequest(
systemInstruction: .init(parts: [.init(text: systemPrompt)]),
contents: [.init(role: "user", parts: [.init(text: "(userStatsHeader)n(statsContext)")])],
...
// [5.1.3] Privacy: do NOT send the user's body weight (Health/Fitness data) to the server.
// Body weight stays on-device. The AI uses generic guidelines and the user's experience
// level (passed via system prompt) to recommend loads. Local UI can scale weights using
// userProfile.weightKg after the response.
...
(catalogContext)
"""
let userPrompt = "Current workout:n(workoutContext)nnUser command: (commandType)"
let requestBody = GeminiRequest(
systemInstruction: .init(parts: [.init(text: systemPrompt)]),
...
let requestBody = GeminiRequest(
systemInstruction: .init(parts: [.init(text: systemPrompt)]),
contents: [.init(role: "user", parts: [.init(text: "CONTEXT:n(workoutContext)nUSER: "(userMessage)"")])],
...
FOCUS HEAVILY on growing: (musclesToGrow.isEmpty ? "None specified" : musclesToGrow.joined(separator: ", ")).
EXCLUDE exercises targeting: (musclesToExclude.isEmpty ? "None specified" : musclesToExclude.joined(separator: ", ")).
...
let requestBody = GeminiRequest(
systemInstruction: .init(parts: [.init(text: systemPrompt)]),
contents: [.init(role: "user", parts: [.init(text: "WORKOUT SUMMARY:n(workoutContext)")])],
...
// [5.1.3] Privacy: system prompt intentionally does NOT include the user's body weight.
// Use experience level + recent PRs only to scale recommendations.
var prompt = """
(customInstruction)
...
prompt += "nОТВЕЧАЙ СТРОГО НА РУССКОМ ЯЗЫКЕ (кроме названий упражнений)."
}
if !profile.availableExercises.isEmpty {
prompt += "nAVAILABLE EXERCISES FOR WORKOUTS:n(profile.availableExercises.joined(separator: ", "))"
}
if !profile.recentPRs.isEmpty {
let prSummary = profile.recentPRs.map { "($0.key): ($0.value) kg" }.joined(separator: ", ")
prompt += "nRECENT PRs (use to calibrate intensity):n(prSummary)"
}
if !profile.fatiguedMuscles.isEmpty {
prompt += "nFATIGUED MUSCLE GROUPS (avoid heavy loading): (profile.fatiguedMuscles.joined(separator: ", "))"
}
return prompt
}
}
