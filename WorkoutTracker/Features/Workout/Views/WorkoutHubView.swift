            ...
            .sheet(isPresented: $showSmartBuilder) {
                         SmartGeneratorEntryView(onWorkoutReady: { exerciseDTOs in
                             Task { @MainActor in
                                 ...
                                 // ✅ FIX: 1. Map the DTOs safely without touching the MainContext
                                 let generatedDTO = GeneratedWorkoutDTO(
                                     ...
                                     exercises: exerciseDTOs.map { dto in
                                         GeneratedExerciseDTO(
                                             ...
                                             sets: dto.setsList.count,
                                             reps: dto.setsList.first?.reps ?? 10,
                                             recommendedWeightKg: dto.setsList.first?.weight,
                                             restSeconds: nil
                                         )
                                     }
                                 )
                                 
                                 // ✅ FIX: 2. Offload the heavy CoreData/SwiftData insertion to the Background Actor
                                 await workoutService.startGeneratedWorkout(generatedDTO)
                                 
                                 // ✅ FIX: 3. Safely fetch the newly generated workout and navigate
