...
final class AICoachViewModel {
    ...
    // ✅ ИСПРАВЛЕННЫЙ МЕТОД SEND MESSAGE
    func sendMessage(userWeight: Double, uiText: String? = nil, aiPrompt: String? = nil, isExplicitWorkoutRequest: Bool = false) async {
        ...
        if isExplicitWorkoutRequest {
            // Запрос пришел с кнопки (например, из Smart Builder)
            await requestWorkoutPlan(prompt: actualPrompt, userWeight: userWeight)
        } else {
            // Юзер напечатал текст. Спрашиваем ИИ: это запрос на тренировку?
    ...
    // ✅ НОВЫЙ МЕТОД ДЛЯ ПОТОКОВОГО ЧАТА
    private func streamChat(prompt: String, userWeight: Double) async {
           ...
           currentTask = Task {
               do {
                   ...
                   // Ищем индекс нашего пустого сообщения
                   ...
                   for try await chunk in stream {
                       ...
                       // Обновляем текст прямо в массиве
                       withAnimation(.linear(duration: 0.1)) {
                           self.chatHistory[messageIndex].text = fullText
                       }
                   }
                   
                   // Сохраняем в БД полное сообщение
                   ...
                   self.isGenerating = false
                   // Если произошла ошибка, удаляем пустое сообщение ИИ из чата
    ...
    // СТАРЫЙ МЕТОД (Запрашивает JSON-карточку тренировки)
