...
// MARK: - МЕНЕДЖЕРЫ И УТИЛИТЫ (Оптимизация)
class HapticManager {
...
// Удобный модификатор для "стеклянных" карточек
...
// MARK: - БЕЗОПАСНЫЕ ОБЕРТКИ И МОДЕЛИ ДАННЫХ
struct IdentifiableString: Identifiable {
    let id = UUID(); let value: String
}
...
struct AICoachView: View {
    ...
    // Получаем состояние ЦНС для визуализации сферы
    ...
    var body: some View {
        ZStack {
            ...
            DotGridBackground() // Визуал: Сетка
            FloatingParticles()
            ...
            VStack {
                        ...
                        Text("Атлет")
                            ...
                            .overlay(
                                LinearGradient(colors: [.clear, .white, .clear], startPoint: .leading, endPoint: .trailing)
                                    ...
                                    .mask(Text("Атлет").font(.system(size: 32, weight: .black, design: .rounded)))
                            )
                    }
                    Spacer()
                ...
                VStack(spacing: 16) { // Было 28, теперь элементы сгруппированы плотнее
                    Button(action: {
                    ...
                    .accessibilityLabel("ИИ Нейро-тренер")
                    .accessibilityAddTraits(.isButton)
                    
                    VStack(spacing: 4) { // Было 8
                    ...
                    HStack {
                        ...
                        TextField("План питания, техника...", text: $userQuery).foregroundColor(.white)
                            .submitLabel(.send)
                        if !userQuery.isEmpty { Button(action: { userQuery = "" }) { Image(systemName: "xmark.circle.fill").foregroundColor(.gray) } }
                    }
                    ...
                    // Убрал .padding(.top, 5), чтобы поднять поле выше
                    
                    ScrollView(.horizontal, showsIndicators: false) {
                        LazyHStack(spacing: 12) {
                            Spacer().frame(width: 12)
                            ForEach(quickPrompts, id: \.self) { prompt in
                                Button(action: { userQuery = prompt; HapticManager.shared.selection() }) {
                ...
                .padding(.bottom, 12) // Добавлен отступ снизу, чтобы приподнять весь блок над Dock
        ...
        .dynamicTypeSize(.medium ... .accessibility1) // Оптимизация шрифтов
        .onAppear {
            Task {
...
struct ProgressAnalysisSheet: View {
    ...
    @State private var selectedMuscleTip: IdentifiableString? = nil
    @State private var activeSegment: UUID? = nil // Интерактивность графика
    ...
    var body: some View {
        NavigationStack {
                    ...
                    LazyVStack(alignment: .leading, spacing: 30) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            Button(action: {
                                ...
                                if let targetName = target?.name { selectedMuscleTip = IdentifiableString(value: targetName) }
            ...
            .sheet(item: $selectedMuscleTip) { wrapped in BestExercisesSheet(muscle: wrapped.value).presentationDetents([.fraction(0.85)]).presentationCornerRadius(35).presentationDragIndicator(.visible) }
        }
    }
}
        ...
        VStack(spacing: 6) {
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    Capsule().fill(Color.white.opacity(0.05)).frame(height: 6) // Визуал: Фон прогресс-бара
                    Capsule()
...
struct WorkoutConfigSheet: View {
    ...
    func synergyFor(_ muscle: MuscleGroup) -> String? {
...
struct BestExercisesSheet: View {
    ...
    @State private var isSaved = false // Состояние сохранения
        ...
        VStack(spacing: 0) {
            ...
            ScrollView(showsIndicators: false) {
                LazyVStack(spacing: 16) {
                    // Визуал: Шиммер и пульсация на ожидании
                    HStack {
                    ...
                    .shimmering(active: isGenerating) // Кастомный модификатор
                    ...
                    ForEach(Array(data.enumerated()), id: \.offset) { index, rec in
                        ...
                        .redacted(reason: isGenerating ? .placeholder : [])
                        // Оптимизация: Scroll Transitions (iOS 17+)
...
// Модификатор мерцания для скелетона
extension View {
    @ViewBuilder func shimmering(active: Bool) -> some View {
        if active { self.modifier(PulseEffect()) } else { self }
    }
}
...
struct RestAnalysisSheet: View {
    ...
    @AppStorage("cnsScore") private var cnsScore: Double = 85.0 // Связываем с Main View
                ...
                ScrollView(showsIndicators: false) {
                    LazyVStack(alignment: .leading, spacing: 32) {
                        ...
                        VStack(alignment: .leading, spacing: 20) {
                            ...
                            VStack(spacing: 12) {
                                // ИСПРАВЛЕНИЕ: Используем корректный инициализатор Image с variableValue и правильное имя "drop.fill"
    ...
    private func updateCNS() { cnsScore = 100 - cnsLoad } // Обновляем глобальный стейт
}

// MARK: - ВСПОМОГАТЕЛЬНЫЕ КОМПОНЕНТЫ И ЭФФЕКТЫ
...
struct ScaleButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View { configuration.label.scaleEffect(configuration.isPressed ? 0.9 : 1.0).opacity(configuration.isPressed ? 0.8 : 1.0).animation(.spring(response: 0.3, dampingFraction: 0.6), value: configuration.isPressed) }
}
struct ParallaxButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View { configuration.label.scaleEffect(configuration.isPressed ? 0.95 : 1.0).rotation3DEffect(.degrees(configuration.isPressed ? 5 : 0), axis: (x: 1, y: 0, z: 0)).opacity(configuration.isPressed ? 0.9 : 1.0).animation(.spring(response: 0.3, dampingFraction: 0.5), value: configuration.isPressed) }
}
struct PulseEffect: ViewModifier {
    @State private var isPulsing = false
    func body(content: Content) -> some View { content.scaleEffect(isPulsing ? 1.2 : 0.8).opacity(isPulsing ? 0.5 : 1.0).onAppear { withAnimation(.easeInOut(duration: 1).repeatForever(autoreverses: true)) { isPulsing = true } } }
}

struct HistoryBreathingBackground: View {
    @State private var phase = false
    var cnsScore: Double // Визуал: Динамический Mesh Gradient
...
struct FloatingParticles: View {
    ...
    var body: some View {
        ZStack {
            ForEach(0..<8, id: \.self) { i in // Оптимизация: меньше частиц
                Circle()
...
struct CustomDonutChart: View {
    ...
    var body: some View {
        ...
        }.drawingGroup() // Оптимизация рендеринга
    }
}
