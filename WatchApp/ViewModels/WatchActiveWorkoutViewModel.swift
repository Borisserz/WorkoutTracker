...
final class WatchActiveWorkoutViewModel: Hashable {
    ...
    var totalSets: Int = 0
    var totalDurationSeconds: Int = 0 // 🛠️ FIX: Переменная для времени тренировки
    ...
    func initializeWorkout() async {
        ...
        // ✅ ИСПРАВЛЕНИЕ: Отправляем полный DTO с упражнениями на iPhone
        let payload = LiveSyncPayload(
            action: .startWorkout,
            workoutID: workoutID.uuidString,
            workoutTitle: workoutTitle,
            exercises: self.exercises // <--- Передаем все упражнения из пресета
        )
        ...
        private func applyRemoteDelta(_ payload: LiveSyncPayload) {
            ...
            case .syncFullState:
                            if let newExercises = payload.exercises {
                                ...
                                // ✅ ЗАЩИТА: Если на телефоне удалили упражнение, которое сейчас открыто на часах — выходим в общее меню
    ...
    func saveRPE(_ rpe: Int) async {
        ...
        // ✅ ИСПРАВЛЕНИЕ: Отправляем событие на iPhone
    ...
    func finishWorkout(activeEnergy: Double) async {
           ...
           // ✅ ИСПРАВЛЕНИЕ: Отправляем полную тренировку и калории на iPhone
           let payload = LiveSyncPayload(
